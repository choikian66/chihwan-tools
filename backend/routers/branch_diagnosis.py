...
from utils.db import safe_db  # type: ignore
    ...
    with safe_db() as (_, cur):
        cur.execute("""
            SELECT DISTINCT ON (branch_name)
                ...
                (SELECT COUNT(*) FROM diagnosis_items
                 WHERE diagnosis_id = bd.id AND checked = FALSE
                 AND 개선예정일 <> '' AND 개선예정일 < CURRENT_DATE::text) AS overdue_count
