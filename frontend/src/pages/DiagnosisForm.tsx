...
function RadarChart({ catStats }: { catStats: Record<string, { total: number; checked: number }> }) {
  ...
  const rates = CATEGORIES.map(cat => {
    ...
    return st.total ? st.checked / st.total : 0;
  });
  const pt = (i: number, frac = 1) => [
    cx + R * frac * Math.cos(angles[i]),
    cy + R * frac * Math.sin(angles[i]),
  ];
  const gridPoly = (frac: number) =>
    CATEGORIES.map((_, i) => pt(i, frac).join(',')).join(' ');
  const dataPath =
    CATEGORIES.map((_, i) => {
      const [x, y] = pt(i, rates[i]);
  ...
  return (
    <svg width={SIZE} height={SIZE} style={{ flexShrink: 0 }}>
      {[0.25, 0.5, 0.75, 1].map(f => (
        <polygon key={f} points={gridPoly(f)}
          fill="none" stroke="var(--border-primary)" strokeWidth={f === 1 ? 1 : 0.5} />
      ))}
      {CATEGORIES.map((_, i) => {
        const [x2, y2] = pt(i);
      ...
      <path d={dataPath} fill="#5B5FC7" fillOpacity={0.15}
        stroke="#5B5FC7" strokeWidth={2} strokeLinejoin="round" />
      {CATEGORIES.map((_, i) => {
        const [x, y] = pt(i, rates[i]);
        return <circle key={i} cx={x} cy={y} r={3} fill="#5B5FC7" />;
      })}
    </svg>
  );
}
  ...
  return (
    <div className={s.container}>
      ...
      <div className={s.radarSection}>
        ...
        <div className={s.catLegend}>
          {CATEGORIES.map(cat => {
            const st = catStats[cat] ?? { total: 0, checked: 0 };
            const rate = st.total ? Math.round(st.checked / st.total * 100) : 0;
            return (
              <div key={cat} className={s.catLegendRow}>
                <span className={s.catDot} style={{ background: CAT_COLOR[cat] }} />
                <span className={s.catName}>{cat}</span>
                <span className={s.catRate}>{rate}%</span>
                <span className={s.catFraction}>({st.checked}/{st.total})</span>
              </div>
            );
          })}
        </div>
          ...
          return (
            <div key={sub} className={s.subGroup}>
              ...
              {!isCollapsed && (subItems as DiagItem[]).map(item => {
                ...
                const isOverdue = !item.checked && !!item.개선예정일 && item.개선예정일 < TODAY;
                return (
                  <div key={item.id} className={`${s.item} ${item.checked ? s.itemChecked : ''} ${isOverdue ? s.itemOverdue : ''}`}>
                    <div
                    ...
                    >
                      ...
                      {isOverdue && <span className={s.overdueBadge}>납기 초과</span>}
                      {compLabel && (
                      ...
                      <div className={s.itemDetail} onClick={e => e.stopPropagation()}>
                        ...
                        {!item.checked && (
                          <div className={s.actionPlanRow}>
                            ...
                            <input
                              type="date"
                              className={`${s.dueDateInput} ${isOverdue ? s.overdueDate : ''}`}
                              value={item.개선예정일}
